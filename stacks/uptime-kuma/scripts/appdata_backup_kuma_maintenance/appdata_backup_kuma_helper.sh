...
KUMA_MAINTENANCE_WINDOW_MINUTES="${KUMA_MAINTENANCE_WINDOW_MINUTES:-30}"
...
STATE_WINDOW_END_EPOCH=${window_end_epoch}
STATEEOF
}
...
run_kuma_socket_action() {
  ...
  docker exec -i \
    ...
    -e KUMA_WINDOW_START="${KUMA_WINDOW_START:-}" \
    -e KUMA_WINDOW_END="${KUMA_WINDOW_END:-}" \
...
const windowStart = process.env.KUMA_WINDOW_START || "";
const windowEnd = process.env.KUMA_WINDOW_END || "";
  ...
  try {
    ...
    if (action === "create") {
      if (!windowStart || !windowEnd) {
        throw new Error("KUMA_WINDOW_START and KUMA_WINDOW_END are required for create");
      }

      // A "single" window carries its own start and end date. Kuma derives the
      // status from those dates every time it is asked, so the window ends on
      // its own even if this run never gets to delete it, and even across a
      // container restart. A "manual" window has no end and is what stranded
      // monitors for months.
      const maintenance = {
        title,
        description,
      ...
      if (!maintenanceID) {
        throw new Error("Maintenance ID is required for delete");
      }

      // Detach the monitors first so nothing depends on the foreign key
      // cascade, then drop the row. The monitors leave maintenance on their
      // next heartbeat because Kuma reads the mapping from the database.
      ...
      await emitAck(socket, "deleteMaintenance", Number(maintenanceID));
      ...
      // The window is created active with its own dates, so there is nothing
      // to resume: attaching the monitors is what puts them under maintenance.
      ...
      // An empty set releases every monitor without touching the window
      // itself; the window is deleted by the stop path once the run is over.
      await emitAck(socket, "addMonitorMaintenance", Number(maintenanceID), monitorIds.map((id) => ({ id })));
...
create_kuma_maintenance() {
  ...
  announce_stderr "kuma: creating single maintenance '${KUMA_MAINTENANCE_TITLE}' window ${KUMA_WINDOW_START} .. ${KUMA_WINDOW_END}"
  if ! output="$(run_kuma_socket_action_with_retry create)"; then
    announce_stderr "kuma: failed to create maintenance '${KUMA_MAINTENANCE_TITLE}'"
    return 1
  fi
  ...
  announce_stderr "kuma: created maintenance id=${maintenance_id}"
  printf '%s' "$maintenance_id"
}

delete_kuma_maintenance() {
  local maintenance_id="$1"
  local heartbeat_csv="${2:-}"

  if ! run_kuma_socket_action_with_retry delete "$maintenance_id" "" "$heartbeat_csv" >/dev/null; then
    announce "kuma: failed to delete maintenance id=${maintenance_id}"
    return 1
  fi

  announce "kuma: deleted maintenance id=${maintenance_id}"
  return 0
}
...
build_release_plan_json() {
  ...
  jq -cn \
    ...
    # Only monitors whose readiness this script can actually confirm after the
    # run may enter the window. A push monitor has nothing to probe, so it
    # would either be released blind or stay stuck; it stays armed instead.
    | ($unmapped_all | map(select((.type == "http" and (.url | length) > 0) or .type == "dns")) | sort_by(.id)) as $unmapped_monitors
    | ($unmapped_monitors | map(.id)) as $unmapped_ids
    | ($unmapped_all | map(select(.id as $mid | $unmapped_ids | index($mid) | not)) | sort_by(.id)) as $skipped_monitors
    | ($mapped_ids + $unmapped_ids | unique | sort) as $eligible_ids
    | {
        selected_monitor_ids: $eligible_ids,
        active_monitor_ids: ($selected | map(select(.active == true and (.id as $mid | $eligible_ids | index($mid)) != null) | .id) | unique | sort),
        containers: $container_plan,
        unmapped_monitor_ids: ($unmapped_ids | unique | sort),
        unmapped_monitors: $unmapped_monitors,
        skipped_monitors: $skipped_monitors
      }
  '
}
...
# Every run owns exactly one window: it creates it, it deletes it. Nothing is
# reused by title, so two overlapping runs never fight over the same row and a
# leftover window from a crashed run expires on its own dates.
ensure_kuma_maintenance() {
  ...
  maintenance_id="$(create_kuma_maintenance)" || return 1
  printf '%s' "$maintenance_id"
}
...
log_release_plan() {
  ...
  if ! json_array_is_empty "$(jq -c '.skipped_monitors' <<< "$release_plan_json")"; then
    announce "kuma: monitors left armed because their readiness cannot be verified ids=$(jq -r '.skipped_monitors | map(.id | tostring) | join(",")' <<< "$release_plan_json") names=$(jq -r '.skipped_monitors | map(.name) | join(",")' <<< "$release_plan_json")"
  fi
}

# Renders an epoch as the local "YYYY-MM-DD HH:MM:SS" Kuma expects. The
# container runs with the same TZ as the host, and the window is created with
# timezoneOption SAME_AS_SERVER, so host local time is the right frame.
format_window_stamp() {
  date -d "@$1" '+%Y-%m-%d %H:%M:%S'
}

start_action() {
  local maintenance_id monitors_json docker_containers_json release_plan_json
  local monitor_ids_json active_monitor_ids_json
  local monitor_ids active_monitor_ids monitor_count active_monitor_count
  local window_minutes start_epoch end_epoch

  require_command docker || return 1
  require_command jq || return 1

  window_minutes="$KUMA_MAINTENANCE_WINDOW_MINUTES"
  if ! [[ "$window_minutes" =~ ^[1-9][0-9]*$ ]]; then
    announce "kuma: KUMA_MAINTENANCE_WINDOW_MINUTES='${window_minutes}' is not a positive integer, falling back to 30"
    window_minutes=30
  fi

  monitors_json="$(fetch_kuma_monitors_json)" || return 1
  docker_containers_json="$(fetch_docker_containers_json)" || return 1
  release_plan_json="$(build_release_plan_json "$monitors_json" "$docker_containers_json")" || return 1
  log_release_plan "$release_plan_json"

  monitor_ids_json="$(jq -c '.selected_monitor_ids' <<< "$release_plan_json")"
  ...
  if [[ -z "$monitor_ids" ]]; then
    announce "kuma: no verifiable monitors to suppress, skipping maintenance start"
    clear_state
    return 0
  fi

  # Start the window a minute in the past so a clock skew of a few seconds
  # between this shell and the Kuma container cannot open a gap in which the
  # monitors are attached but the window does not count as running yet.
  start_epoch=$(( $(date +%s) - 60 ))
  end_epoch=$(( start_epoch + 60 + window_minutes * 60 ))
  KUMA_WINDOW_START="$(format_window_stamp "$start_epoch")"
  KUMA_WINDOW_END="$(format_window_stamp "$end_epoch")"

  maintenance_id="$(ensure_kuma_maintenance)" || return 1

  # State is written before the monitors are attached: if attaching dies
  # halfway, the stop path still knows which window to clear and delete.
  save_state "$maintenance_id" "$end_epoch"

  announce "kuma: starting maintenance id=${maintenance_id} until ${KUMA_WINDOW_END} for monitors count=${monitor_count} ids=${monitor_ids}"
  if ! run_kuma_socket_action_with_retry start_bundle "$maintenance_id" "$monitor_ids" "$active_monitor_ids" >/dev/null; then
    ...
    delete_kuma_maintenance "$maintenance_id" || true
    clear_state
    return 1
  fi
...
# The single exit of the stop path: detach every monitor, delete the window
# this run created, and drop the state file. Called on every outcome, so a run
# can never end with its own window still standing.
finish_stop() {
  local maintenance_id="$1"
  local heartbeat_ids_json="${2:-[]}"

  if ! delete_kuma_maintenance "$maintenance_id" "$(json_array_join_csv "$heartbeat_ids_json")"; then
    return 1
  fi
...
stop_action() {
  ...
  if json_array_is_empty "$remaining_json"; then
    announce "kuma: no monitors left to release, removing maintenance id=${STATE_MAINTENANCE_ID}"
    finish_stop "$STATE_MAINTENANCE_ID" "$active_monitor_ids_json"
    return $?
  fi

  # The poll never outlives the window itself. Once the window's end date has
  # passed, Kuma has stopped suppressing anyway, so waiting longer buys nothing
  # and the run must not leave the row behind.
  timeout_seconds=$(( STATE_WINDOW_END_EPOCH - $(date +%s) ))
  if (( timeout_seconds < 0 )); then
    timeout_seconds=0
  fi

  announce "kuma: waiting up to ${timeout_seconds}s for per-container readiness before clearing maintenance id=${STATE_MAINTENANCE_ID}"
  ...
  while true; do
    ...
    if [[ "$remaining_count" == "0" ]]; then
      announce "kuma: every monitor released, removing maintenance id=${STATE_MAINTENANCE_ID}"
      finish_stop "$STATE_MAINTENANCE_ID" "$active_monitor_ids_json"
      return $?
    fi

    if (( SECONDS >= deadline_seconds )); then
      announce "kuma: maintenance window ended while monitors were still unconfirmed, removing maintenance id=${STATE_MAINTENANCE_ID} and re-arming monitor ids=$(json_array_join_csv "$remaining_json")"
