...
# Start AutoKuma again. Deliberately before the maintenance is lifted and not
# guarded by the helper's exit status: a failed pre-run stop or a failed
# maintenance stop must never leave AutoKuma down.
AUTOKUMA_CONTAINER_NAME="${AUTOKUMA_CONTAINER_NAME:-autokuma}"
if ! docker start "$AUTOKUMA_CONTAINER_NAME" >/dev/null 2>&1; then
  printf '%s backup-kuma: could not start %s, start it by hand\n' "$(date '+%F %T')" "$AUTOKUMA_CONTAINER_NAME" >&2
fi
