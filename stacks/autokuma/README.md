...
- **Ownership split.** 31 monitors are label-managed by this stack. 16 monitors (11 push, 4 dns, 1 http) stay hand-managed for now, because nothing container-shaped exists behind them or because their `pushToken` is an identity a rebuild would destroy. All 30 are migrated: `registry` in wave one (#1776), the ten `docker` monitors in wave two (#1798), `traefik - traefik` plus `adguard-home-unbound - adguard-home-sync` in wave 3a (#1858), and the remaining seventeen `http` monitors in one pass as wave 3b (#1861). The 31st is `qbittorrent - bandwidth-share`, which was never hand-built at all: it is the first `push` monitor AutoKuma owns, created straight from its label. The migration backlog is empty; a new monitor is now created by writing its label, not by clicking it together in the Kuma UI.
