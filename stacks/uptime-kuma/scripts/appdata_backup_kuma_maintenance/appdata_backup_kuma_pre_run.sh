...
# Stop AutoKuma for the same window. It reconciles monitors from container
# labels, so while the backup has the containers stopped it would see the
# labels disappear and delete the very monitors this maintenance is protecting.
AUTOKUMA_CONTAINER_NAME="${AUTOKUMA_CONTAINER_NAME:-autokuma}"
if ! docker stop "$AUTOKUMA_CONTAINER_NAME" >/dev/null 2>&1; then
  printf '%s backup-kuma: could not stop %s, continuing backup\n' "$(date '+%F %T')" "$AUTOKUMA_CONTAINER_NAME" >&2
fi
